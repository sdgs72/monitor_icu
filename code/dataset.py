...
class MimicDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # dxh_file_path = './preprocess/data/train_interval1_data.npy'
        ...
        if len(self.positives) == 0 or len(self.negatives) == 0:
            raise AssertionError(
                "No Positive or Negative Data generated please check....")
    ...
    def _aggregate_raw_data(self):
        ...
        logging.info("Preprocess dataset with block size of %d",
                     self.block_size)
        aggregated_data = {}
    ...
    def resample(self):
        ...
        logging.info(f"DXH _sample_data output list {len(sample_list)}")
    ...
    def _preprocessing(self):
        ...
        aggregated_data = np.stack([np.sum(x, axis=0) for x in self.data.values()],
                                   axis=0)
        logging.info("Number of samples: %d, Feature dimension before PCA: %d",
                     aggregated_data.shape[0], aggregated_data.shape[1])
        ...
        if self.phase == "training":
            ...
            raise ValueError(
                "Only `training` and `inference` are supported `phase`.")
    ...
    def _generate_candidates(self):
        ...
        # print(self.labels[self.target_label].items())
        ...
        for hadm_id, label_time in self.labels[self.target_label].items():
            ...
            for start_time in range(0, dxh_finol):
                ...
                [negatives, positives][label].append(
                    (hadm_id, history_window, label))
    ...
    def _sample_data(self):
        if self.dataset_size == -1:
            ...
            logging.info("Dataset use default size: %d",
                         len(self.positives) * 2)
    ...
    def _load_labels(self):
        ...
        with open(os.path.join(self.data_dir, self.label_name)) as fp:
            ...
            for row in reader:
                ...
                labels["discharge"][hadm_id] = _hour_to_block(
                    row["dischargeTime"])
