CREATE TABLE gcs_first_day (    
    ...
    mingcs mediumint unsigned NULL
)


insert into gcs_first_day 
with base as
(
...
, gcs as (
  ...
  from base b
  left join base b2   -- join to itself within 6 hours to get previous value
    on b.ICUSTAY_ID = b2.ICUSTAY_ID and b.rn = b2.rn+1 and b2.charttime > DATE_SUB(b.charttime, INTERVAL 6 HOUR)
)
, gcs_final as (
  select gcs.*
