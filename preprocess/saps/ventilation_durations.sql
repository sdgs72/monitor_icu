...
CREATE TABLE ventilation_durations (    
    ...
    ventnum mediumint unsigned NOT NULL,
    starttime datetime NOT NULL,
    endtime datetime NOT NULL,
    duration_hours mediumint unsigned NOT NULL
);
...
insert into ventilation_durations (
with vd0 as
(
  select
    icustay_id
...
(
  select
      ...
      , case
          ...
          when MechVent=1 then
            DATEDIFF(CHARTTIME, charttime_lag)*24 -- in hours
          else null
        end as ventduration
...
select icustay_id
  ...
  , DATEDIFF(max(charttime), min(charttime))*24 AS duration_hours
from vd2
